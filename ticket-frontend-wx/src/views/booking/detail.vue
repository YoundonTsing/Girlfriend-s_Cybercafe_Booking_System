...
export default {
  ...
  methods: {
    ...
    // 根据时长获取正确的票档ID
    getTicketIdByDuration(duration) {
      // 根据数据库中的票档数据映射
      const ticketMapping = {
        2: 1,  // 2小时 -> 票档ID 1
        3: 2,  // 3小时 -> 票档ID 2  
      ...
      return ticketMapping[duration] || 1 // 默认返回票档ID 1
    },

    // 解析座位ID字符串
    parseSeatIds(seatIdsString) {
      if (!seatIdsString) return []
      return seatIdsString.split(',').map(id => Number(id.trim())).filter(id => !isNaN(id))
    },
    ...
    confirmBooking() {
      ...
      this.$refs.bookingForm.validate((valid) => {
        ...
        const bookingData = {
          ...
          // 根据时长确定正确的票档ID (1-12)
          ticketId: this.getTicketIdByDuration(this.selectedTimeSlot.duration || 2),
          quantity: Number(this.orderForm.quantity),
          bookingDate: this.orderForm.bookingDate,
          bookingEndTime: this.selectedTimeSlot.endTime,
          ...
          // 如果是座位预约，传递座位ID
          seatId: this.selectedTimeSlot.seatIds ? this.parseSeatIds(this.selectedTimeSlot.seatIds)[0] : null
        }
        ...
        // 验证票档ID范围 (1-12)
        if (bookingData.ticketId < 1 || bookingData.ticketId > 12) {
          this.$message.error('票档ID无效，请重新选择时长')
          this.orderLoading = false
          return
        }

        // 验证座位ID范围 (如果有座位预约)
        if (bookingData.seatId && bookingData.seatId < 31) {
          this.$message.error('座位ID无效，请重新选择座位')
          this.orderLoading = false
          return
        }
