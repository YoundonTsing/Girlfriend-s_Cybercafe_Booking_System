<template>
  <div class="seat-selection-container">
    ...
    <!-- 座位状态实时显示 -->
    <div class="seat-status-panel" v-if="currentLayout.seats && currentLayout.seats.length > 0">
      <div class="status-header">
        <h4>座位状态实时监控</h4>
        <div class="sync-indicator" :class="{ 'syncing': isSyncing }">
          <i class="el-icon-refresh" v-if="isSyncing"></i>
      ...
      <div class="seat-statistics">
        <div class="stat-item">
          ...
          <span class="stat-value">{{ seatStatistics.total }}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">可选</span>
          <span class="stat-value available">{{ seatStatistics.available }}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">已选</span>
          <span class="stat-value selected">{{ seatStatistics.selected }}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">锁定</span>
          <span class="stat-value locked">{{ seatStatistics.locked }}</span>
        </div>
        <div class="stat-item">
          <span class="stat-label">占用</span>
          <span class="stat-value occupied">{{ seatStatistics.occupied }}</span>
        </div>
      </div>
    </div>
    ...
    <div class="seat-legend" v-if="currentLayout.seats && currentLayout.seats.length > 0">
      ...
      <div class="legend-item">
        <div class="seat-icon locked-by-others"></div>
        <span>他人锁定</span>
      </div>
      <div class="legend-item">
...
const seatStatistics = computed(() => {
  if (!currentLayout.value.seats) {
    return { total: 0, available: 0, selected: 0, locked: 0, occupied: 0 }
  }
  
  const stats = {
    total: currentLayout.value.seats.length,
    available: 0,
    selected: 0,
    locked: 0,
    occupied: 0
  ...
  currentLayout.value.seats.forEach(seat => {
    switch (seat.status) {
      case 1:
        stats.available++
        break
...
const handleSeatClick = async (seat) => {
  ...
  if (isSelected) {
    ...
    try {
      ...
      ElMessage.success(`已取消选择座位 ${seat.seatNum}`)
    } catch (error) {
      console.error('取消选择座位失败:', error)
      ElMessage.error('取消选择失败，请重试')
    }
  } else {
    // 选择座位
    try {
      ...
      if (success && success.data) {
        ...
        ElMessage.success(`已选择座位 ${seat.seatNum}`)
      } else {
        ElMessage.warning('座位已被其他用户选择')
      }
    } catch (error) {
      console.error('选择座位失败:', error)
      ElMessage.error('选择座位失败，请重试')
    }
  }
}
...
const syncSeatStatus = async () => {
  if (!currentAreaId.value || isSyncing.value) return
  ...
  try {
    ...
    if (newLayout.seats && currentLayout.value.seats) {
      // 合并状态：保持用户选择的座位状态，更新其他座位
      newLayout.seats.forEach(newSeat => {
        const isUserSelected = selectedSeats.value.some(s => s.id === newSeat.id)
        if (!isUserSelected) {
          const oldSeat = currentLayout.value.seats.find(s => s.id === newSeat.id)
          if (oldSeat && oldSeat.status !== newSeat.status) {
            // 状态发生变化，推送通知
            pushSeatStatusNotification(newSeat, oldSeat.status, newSeat.status)
          }
        }
      })
    ...
    console.error('同步座位状态失败:', error)
  } finally {
    isSyncing.value = false
  }
}

const pushSeatStatusNotification = (seat, oldStatus, newStatus) => {
  const statusMap = {
    1: '可选',
    2: '锁定',
    3: '已占用'
  ...
  if (oldStatus !== newStatus) {
    ElMessage.info(`座位 ${seat.seatRow}-${seat.seatCol} 状态变更: ${statusMap[oldStatus]} → ${statusMap[newStatus]}`)
  }
}
</script>
