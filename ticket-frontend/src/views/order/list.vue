        ...
        <template #default="scope">
          <div class="show-info">
            ...
            <div class="show-detail">
              ...
              <p>预约时段：{{ formatBookingTime(scope.row) }}</p>
              <p v-if="scope.row.venue">{{ scope.row.venue }}</p>
              <!-- 显示预约详细信息 -->
              <div v-if="hasBookingInfo(scope.row)" class="booking-details">
                <p v-if="getBookingDuration(scope.row)" class="duration-info">
                  <i class="el-icon-timer"></i>
                  时长：{{ getBookingDuration(scope.row) }}小时
                </p>
                <p v-if="getSeatInfo(scope.row)" class="seat-info">
                  <i class="el-icon-position"></i>
                  {{ getSeatInfo(scope.row) }}
                </p>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    // 新增：格式化预约时间，支持多种字段
    formatBookingTime(order) {
      // 优先使用预约相关字段
      const bookingTime = order.bookingDate || order.booking_date || order.startTime || order.showTime
    ...
    // 新增：检查是否有预约信息
    hasBookingInfo(order) {
      return !!(
        order.bookingDate || order.booking_date || order.startTime ||
        order.seatId || order.seatInfo || order.seat_id || 
        order.seat_info || order.seatNumber ||
        order.bookingDuration || order.booking_duration || order.duration
      )
    },
    
    // 新增：获取预约时长信息
    getBookingDuration(order) {
      return order.bookingDuration || order.booking_duration || order.duration
    },
    
    // 新增：获取座位信息
    getSeatInfo(order) {
      if (order.seatInfo || order.seat_info) {
        return order.seatInfo || order.seat_info
      }
      if (order.seatId || order.seat_id || order.seatNumber) {
        return `座位${order.seatId || order.seat_id || order.seatNumber}`
      }
      return null
    }
  }
...
.booking-details i {
  ...
  text-align: center;
  color: var(--ai-nvidia-green);
  text-shadow: 0 0 3px var(--ai-shadow-green);
}
