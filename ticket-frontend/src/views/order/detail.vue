<template>
  <div class="order-detail-container" v-loading="loading">
    <el-card class="order-card" v-if="order">
        ...
        <el-col :xs="24" :sm="16" :md="18" :lg="20">
          <div class="show-info">
              ...
              <div class="show-details">
                ...
                <!-- 网咖预约时间信息 - 突出显示 -->
                <div v-if="order.bookingDate || order.booking_date || order.startTime" class="detail-item booking-time-highlight">
                  <i class="el-icon-time detail-icon"></i>
                  <span class="detail-label">预约时间：</span>
                  <span class="detail-value booking-time">{{ formatDateTime(order.bookingDate || order.booking_date || order.startTime) }}</span>
                </div>
                <div v-if="order.bookingEndTime || order.booking_end_time || order.endTime" class="detail-item booking-time-highlight">
                  <i class="el-icon-time detail-icon"></i>
                  <span class="detail-label">结束时间：</span>
                  <span class="detail-value booking-time">{{ formatDateTime(order.bookingEndTime || order.booking_end_time || order.endTime) }}</span>
                </div>
                <div v-if="order.bookingDuration || order.booking_duration || order.duration" class="detail-item">
                  <i class="el-icon-timer detail-icon"></i>
                  <span class="detail-label">使用时长：</span>
                  <span class="detail-value duration-highlight">{{ order.bookingDuration || order.booking_duration || order.duration }}小时</span>
                </div>
                <div class="detail-item">
                ...
                <!-- 座位信息 - 突出显示 -->
                <div v-if="order.seatId || order.seatInfo || order.seat_id || order.seat_info || order.seatNumber" class="detail-item seat-highlight">
                  <i class="el-icon-position detail-icon"></i>
                  <span class="detail-label">座位号：</span>
                  <span class="detail-value seat-number">{{ order.seatInfo || order.seat_info || `座位${order.seatId || order.seat_id || order.seatNumber}` }}</span>
                </div>
                <div class="detail-item">
      ...
      <!-- 核心预约信息卡片 -->
      <el-divider content-position="center" class="tech-divider" v-if="hasBookingInfo">
        <span class="divider-title">核心预约信息</span>
      </el-divider>
      
      <div class="core-booking-section" v-if="hasBookingInfo">
        <el-row :gutter="24">
          <!-- 预约时间卡片 -->
          <el-col :xs="24" :sm="12" v-if="order.bookingDate || order.booking_date || order.startTime">
            <div class="info-card time-card">
              ...
              <div class="card-content">
                <div class="time-display">{{ formatDateTime(order.bookingDate || order.booking_date || order.startTime) }}</div>
                <div v-if="calculatedEndTime" class="time-range">
                  至 {{ formatDateTime(calculatedEndTime) }}
                </div>
                <div v-if="order.bookingDuration || order.booking_duration || order.duration" class="duration-badge">
                  时长：{{ order.bookingDuration || order.booking_duration || order.duration }}小时
                </div>
              </div>
            </div>
          </el-col>
          ...
          <!-- 座位信息卡片 -->
          <el-col :xs="24" :sm="12" v-if="order.seatId || order.seatInfo || order.seat_id || order.seat_info || order.seatNumber">
            <div class="info-card seat-card">
              <div class="card-header">
                <i class="el-icon-position card-icon"></i>
                <span class="card-title">座位信息</span>
              </div>
              <div class="card-content">
                <div class="seat-display">{{ order.seatInfo || order.seat_info || `座位${order.seatId || order.seat_id || order.seatNumber}` }}</div>
                <div class="seat-type">{{ order.ticketName || '标准座位' }}</div>
              </div>
            </div>
          </el-col>
        </el-row>
      ...
      <!-- 其他预约信息 -->
      <el-divider content-position="center" v-if="order.contactPhone || order.remark">其他信息</el-divider>
      
      <el-row :gutter="20" v-if="order.contactPhone || order.remark">
        <el-col :span="24">
...
export default {
  ...
  computed: {
    order() {
      ...
      if (order) {
        ...
        console.log('预约时间:', order.bookingDate)
        console.log('座位ID:', order.seatId)
        console.log('座位信息:', order.seatInfo)
        console.log('结束时间:', order.bookingEndTime)
        console.log('使用时长:', order.bookingDuration)
      }
      return order
    },
    loading() {
    ...
    hasBookingInfo() {
      ...
      return !!(
        this.order.bookingDate || this.order.booking_date || this.order.startTime ||
        this.order.seatId || this.order.seatInfo || this.order.seat_id || 
        this.order.seat_info || this.order.seatNumber
      )
    },
    calculatedEndTime() {
      ...
      // 获取开始时间
      const startTime = this.order.bookingDate || this.order.booking_date || this.order.startTime
      if (!startTime) return null
      
      // 获取使用时长（小时）
      const duration = this.order.bookingDuration || this.order.booking_duration || this.order.duration
      if (!duration) return null
      
      // 计算结束时间：开始时间 + 使用时长
      const start = new Date(startTime)
      const end = new Date(start.getTime() + duration * 60 * 60 * 1000) // 转换为毫秒
  ...
  methods: {
      ...
      this.$confirm('确定要取消订单吗?', '提示', {
        ...
        this.orderStore.cancelOrder(this.order.orderNo)
          .then(() => {
            ...
            // 取消订单后刷新数据，但保持增强数据
            this.orderStore.fetchOrderDetailByOrderNo(this.order.orderNo).then(() => {
              if (this.enhancedOrder) {
                this.enhancedOrder = {
                  ...this.enhancedOrder,
                  ...this.orderStore.currentOrder,
                  // 保持预约信息不被覆盖
                  bookingDate: this.enhancedOrder.bookingDate,
                  bookingEndTime: this.enhancedOrder.bookingEndTime,
                  bookingDuration: this.enhancedOrder.bookingDuration,
                  seatId: this.enhancedOrder.seatId,
                  seatInfo: this.enhancedOrder.seatInfo
                }
              }
            })
          })
    ...
    // 新增：统一的数据获取方法
    async fetchOrderData(orderNo) {
      try {
        await this.orderStore.fetchOrderDetailByOrderNo(orderNo)
        
        if (this.orderStore.currentOrder && this.orderStore.currentOrder.status === 0) {
          this.startCountdown()
        }
      } catch (error) {
...
/* 座位信息突出显示样式 */
.seat-highlight {
  background: linear-gradient(135deg, rgba(118, 185, 0, 0.1), rgba(118, 185, 0, 0.05)) !important;
...
.booking-time-highlight {
  background: linear-gradient(135deg, rgba(0, 179, 255, 0.1), rgba(0, 179, 255, 0.05)) !important;
  border: 1px solid rgba(0, 179, 255, 0.3) !important;
  border-radius: 8px !important;
  padding: 16px 12px !important;
  margin: 8px 0 !important;
...
.booking-time {
  color: var(--ai-tech-blue) !important;
  font-weight: 700 !important;
  font-size: 18px !important;
  text-shadow: 0 0 8px var(--ai-shadow-blue);
}
...
.time-card {
  border-left: 4px solid var(--ai-tech-blue);
}

.time-card:hover {
  border-left-color: var(--ai-tech-blue);
  box-shadow: 0 8px 25px rgba(0, 179, 255, 0.3);
}

.seat-card {
  border-left: 4px solid var(--ai-nvidia-green);
}

.seat-card:hover {
  border-left-color: var(--ai-nvidia-green);
